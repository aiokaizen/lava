...
from lava.utils import (
    get_entity_logo_filename,
    get_entity_logo_light_filename,
    Result,
    adjust_color,
)
...
class Account(BaseModel):
    ...
    name = models.CharField(
        _("Name"), max_length=256, unique=True, null=True, blank=True
    )
    balance = models.DecimalField(
        _("Balance"), max_digits=14, decimal_places=2, default=0, blank=True
    )
...
class Bank(BaseModel):
    ...
    country = models.CharField(
        _("Country"), max_length=256, default="Morocco", blank=True
    )
...
class Entity(BaseModel):
    ...
    logo = models.ImageField(
        _("Logo"), upload_to=get_entity_logo_filename, null=True, blank=True
    )
    logo_light = models.ImageField(
        _("Logo light"), upload_to=get_entity_logo_light_filename, null=True, blank=True
    )
    ...
    phone_number = models.CharField(
        _("Phone number"), max_length=32, default="", blank=True
    )
    phone_number2 = models.CharField(
        _("Phone numbser 2"), max_length=32, default="", blank=True
    )
    ...
    ifisc = models.CharField(
        _("IF"), max_length=10, null=True, blank=True
    )  # Identifiant fiscale
    tp = models.CharField(_("TP"), max_length=10, null=True, blank=True)
    preferences = models.JSONField(_("Preferences"), default=dict)
    is_current = models.BooleanField(_("Local entity"), default=False, blank=True)
    ...
    def create(self, user=None, check_address=False, *args, **kwargs):
        ...
        return Result.success(
            _(
                "The entity '%(name)s' has been successfully created."
                % {"name": self.name}
            ),
            instance=self,
        )
    ...
    def get_filter_params(cls, kwargs=None):
        ...
        if "query" in kwargs:
            filter_params &= Q(name__icontains=kwargs.get("query"))
