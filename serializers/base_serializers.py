...
class BaseModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data, **kwargs):
        ...
        if self.result.is_error and self.result.errors:
            raise serializers.ValidationError(
                self.result.errors or self.result.message, self.result.error_code
            )
        return instance
    ...
    def update(self, instance, validated_data, **kwargs):
        ...
        self.result = instance.update(
            user=self.user, update_fields=update_fields, m2m_fields=m2m_fields, **kwargs
        )
...
class ReadOnlyBaseModelSerializer(BaseModelSerializer):
    ...
    _READ_ONLY_ERROR_MESSAGE = _(
        "You can not create or update objects using a read-only serializer."
    )
